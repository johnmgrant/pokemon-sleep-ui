  ...
  return (
    <Flex center noFullWidth className={clsx(
      ...
      <Flex direction="row" className="gap-2">
        <Flex center>
          <Link scroll={false} prefetch={false} href={`/pokedex/${pokemonId}`} className="button-clickable group relative h-6 w-6">
            <GenericPokeballIcon alt={t2('Pokedex.Page.Title', {name})} noWrap/>
          </Link>
        </Flex>
        <Flex center>
          <Link scroll={false} prefetch={false} href={`/analysis/${pokemonId}`} className="button-clickable group relative h-6 w-6">
            <NextImage
